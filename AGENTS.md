...
5. Create one branch for one PR-sized slice. Always fetch remote `main`, then rebase the new branch on `origin/main` before implementation. Exception: while a stack is open (see "Stacked pull requests" below), a new slice branches from the top of the stack and is never rebased onto `origin/main`; the stacked rule wins. Never push directly to `main`.
