...
describe("notification API routes", () => {
  ...
  it("returns the public Web Push config and current release summary", async () => {
    ...
    await expect(res.json()).resolves.toMatchObject({
      ...
      releaseNoticeId: "2026-07-16-0.1.272-bunker-visible-ore",
      releaseSummary:
        "See the ore in your bunker walls: Your claimed bunker now shows its ore through the rock, so a fresh claim reads like the mine instead of blank dirt.",
    });
  });
