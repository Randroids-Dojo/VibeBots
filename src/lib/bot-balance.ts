...
/**
 * Balance readouts for the workshop bench: where the mass sits, which parts
 * carry the bot, and how far it can lean before it goes over.
 *
 * This lives in lib rather than src/sim on purpose. It is a UI readout, not
 * part of the deterministic match, and the tip-over angle needs Math.atan2,
 * which the sim purity gate bans (transcendentals differ across engines).
 * It reads the same computeLayout placements the assembly uses, so what it
 * measures is what fights.
 */
...
/** Axis-aligned half extents of a shape, before any yaw quarter-turn. */
function halfExtents(shape: PartShape): { hx: number; hy: number; hz: number } {
...
function contactPoints(
  ...
  const { hx, hy: _hy, hz } = halfExtents(shape);
...
export interface BalanceReport {
  ...
  balanced: boolean;
  /**
   * How far the bot can lean before the centre of mass crosses the
   * footprint edge, in degrees. 0 means it is already on the edge.
   */
  tipOverDegrees: number;
}
...
export function computeBalance(
  ...
  for (const instance of design.parts) {
    ...
    // Quarter-turn yaw swaps the x and z extents (same test as the design
    // overlap check, which keeps every part axis aligned).
    const yawSwapped =
      Math.abs(placement.rotation.y) > 0.6 &&
      Math.abs(placement.rotation.y) < 0.8;
    const { hy } = halfExtents(def.shape);
  ...
  const centerOfMassHeight = Number.isFinite(groundY)
    ...
    : 0;
  // A taller bot tips at a shallower lean for the same footprint; that is
  // the whole reason the tower core feels different to drive.
  const tipOverDegrees =
    stabilityMargin <= 0 || centerOfMassHeight <= 0
      ? 0
      : (Math.atan2(stabilityMargin, centerOfMassHeight) * 180) / Math.PI;
