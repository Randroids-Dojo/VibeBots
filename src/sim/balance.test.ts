...
describe("computeBalance", () => {
  ...
  it("puts a tall chassis mass higher over the same footprint", () => {
    // The geometry behind the archetype difference; the lean angle it
    // implies is asserted in the lib test that owns the conversion.
    ...
    for (const balance of [wedge, tower]) {
      expect(balance.balanced).toBe(true);
      expect(balance.stabilityMargin).toBeGreaterThan(0);
    }
  });
  ...
  it("handles a design with no resolvable parts without throwing", () => {
    ...
    expect(balance.stabilityMargin).toBeLessThanOrEqual(0);
  });
});
