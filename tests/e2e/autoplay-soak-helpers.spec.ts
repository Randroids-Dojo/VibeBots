...
test.describe("blockingOverlay", () => {
  test(
    "returns the first VISIBLE modal, skipping a hidden earlier match",
    ciCase("E2E-AUTOPLAY-0001", "@functional"),
    async ({ page }) => {
  ...
  test(
    "returns null when no modal is visible",
    ciCase("E2E-AUTOPLAY-0002", "@functional"),
    async ({ page }) => {
      await page.setContent(`
...
test.describe("drainKnownDialogs", () => {
  test(
    "dismisses each known dialog through its own control by name",
    ciCase("E2E-AUTOPLAY-0003", "@functional"),
    async ({ page }) => {
  ...
  test(
    "leaves an unknown dialog for the blocking-overlay net",
    ciCase("E2E-AUTOPLAY-0004", "@functional"),
    async ({ page }) => {
      await page.setContent(`
...
test.describe("drainTripReports", () => {
  test(
    "reports nothing to drain when no report is present",
    ciCase("E2E-AUTOPLAY-0005", "@functional"),
    async ({ page }) => {
  ...
  test(
    "drains a dismissable report and does not flag it",
    ciCase("E2E-AUTOPLAY-0006", "@functional"),
    async ({ page }) => {
      await page.setContent(`
  ...
  test(
    "signals stuck and persists when a report stays visible after the bound",
    ciCase("E2E-AUTOPLAY-0007", "@functional"),
    async ({ page }) => {
