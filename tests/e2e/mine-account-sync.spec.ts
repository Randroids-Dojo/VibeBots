...
test("fresh guest play stays account-free until the Account dialog is opened", async ({
  page,
}) => {
...
test("mine account deep link opens the account dialog without sign-in", async ({
  page,
}) => {
...
test("mine account fallback checks account before creating a guest world", async ({
  page,
}) => {
...
test("mine account dialog dismisses from outside tap, Escape, and gamepad back", async ({
  page,
}) => {
...
test("mine account handoff return finishes and clears the callback query", async ({
  page,
}) => {
...
test("mine account handoff retries when Clerk session settles late", async ({
  page,
}) => {
...
test("mine account handoff conflict shows the cloud-save choice", async ({
  page,
}) => {
  ...
  await expect
    .poll(() => page.evaluate(() => window.location.search))
    .not.toContain("accountHandoff");
  await account.getByRole("button", { name: "Keep this device save" }).click();
  await expect(account).not.toBeVisible();
  expect(finishRequests).toBe(1);
...
test("mine account conflict loads the cloud save only after player choice", async ({
  page,
}) => {
