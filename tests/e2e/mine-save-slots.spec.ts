...
test(
  "save slot deletion requires a destructive double confirmation",
  ciCase("E2E-MINE-SAVE-SLOTS-0001", "@functional"),
  async ({ page }) => {
...
test(
  "loading a save slot refreshes the selected world and gear",
  ciCase("E2E-MINE-SAVE-SLOTS-0002", "@functional", ["@critical"]),
  async ({ page }) => {
    let activeSlot = 1;
    let postRequests = 0;
...
test(
  "an active mine trip resumes at the same depth after reload",
  ciCase("E2E-MINE-SAVE-SLOTS-0003", "@functional"),
  async ({ page }) => {
    await page.goto("/mine");
    await dismissReleaseNotes(page);
