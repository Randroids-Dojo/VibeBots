...
test.describe("phone viewport", () => {
  ...
  test(
    "touch drag moves the miner and page pinch stays locked",
    ciCase("E2E-MINE-VIEWPORT-0003", "@functional"),
  ...
  test(
    "held touch movement cancels after a fatal fall reset",
    ciCase("E2E-MINE-VIEWPORT-0004", "@render"),
    async ({ browserName, page }) => {
      test.skip(browserName !== "chromium", "uses CDP touch events");
  ...
  test(
    "surface saves with bunkers still accept movement drags",
    ciCase("E2E-MINE-VIEWPORT-0005", "@functional"),
    async ({ browserName, page }) => {
      test.skip(browserName !== "chromium", "uses CDP touch events");
  ...
  test(
    "control copy never mentions the keyboard on touch devices",
    ciCase("E2E-MINE-VIEWPORT-0006", "@functional"),
    async ({ page }) => {
      await page.goto("/mine");
      await dismissReleaseNotes(page);
  ...
  test(
    "camera pans laterally so mining left stays on screen",
    ciCase("E2E-MINE-VIEWPORT-0007", "@render"),
    async ({ page }) => {
      await page.goto("/mine");
      await dismissReleaseNotes(page);
      const status = page.getByLabel("Mine status");
  ...
  test(
    "walking the surface shop row stays responsive",
    ciCase("E2E-MINE-VIEWPORT-0008", "@render"),
    async ({ page }) => {
      await page.goto("/mine");
      await dismissReleaseNotes(page);
      const status = page.getByLabel("Mine status");
      ...
      expect(
        Number(await canvas.getAttribute("data-frame-ms")),
      ).toBeGreaterThan(0);
  ...
  test(
    "mine posts a compact performance sample",
    ciCase("E2E-MINE-VIEWPORT-0009", "@soak"),
    async ({ page }) => {
  ...
  test(
    "a downward drag from anywhere inside a stall sheet dismisses it",
    ciCase("E2E-MINE-VIEWPORT-0010", "@render"),
    async ({ page }) => {
      await page.goto("/mine");
      await dismissReleaseNotes(page);
      const status = page.getByLabel("Mine status");
      ...
      const upgradesPrompt = page.getByRole("button", {
        name: "Open Upgrades",
      });
      for (let i = 0; i < 12; i++) {
...
test(
  "mine wheel zoom extends into the starter lantern falloff",
  ciCase("E2E-MINE-VIEWPORT-0001", "@render"),
  async ({ page }) => {
...
test(
  "mine HUD zoom buttons adjust the lantern-capped camera",
  ciCase("E2E-MINE-VIEWPORT-0002", "@render"),
  async ({ page }) => {
...
test.describe("Pixel density mine visibility", () => {
  ...
  test(
    "daylight keeps useful visibility at every zoom",
    ciCase("E2E-MINE-VIEWPORT-0011", "@render"),
    async ({ page }) => {
  ...
  test(
    "planet stays camera-locked through a complete surface step",
    ciCase("E2E-MINE-VIEWPORT-0012", "@render"),
    async ({ page }, testInfo) => {
      test.setTimeout(120_000);
      await page.addInitScript(() => {
        (
      ...
      for (let i = 0; i < 50; i++) {
        if (
          Number(await status.getAttribute("data-horizontal-distance")) >= 16
        ) {
          break;
        }
        await pressMineKey(page, "ArrowRight");
  ...
  test(
    "adjacent occupied cells keep their open silhouette",
    ciCase("E2E-MINE-VIEWPORT-0013", "@render"),
    async ({ page }, testInfo) => {
      test.setTimeout(120_000);
      await page.addInitScript(() => {
        (
