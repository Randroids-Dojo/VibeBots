...
test.describe("performance telemetry", () => {
  test(
    "toggle starts trace batches with scene probe data",
    ciCase("E2E-PERF-TELEMETRY-0001", "@soak"),
    async ({ page }) => {
  ...
  test(
    "an enabled toggle persists and collects on other surfaces",
    ciCase("E2E-PERF-TELEMETRY-0002", "@soak"),
    async ({ page }) => {
      const batches: unknown[] = [];
      await page.route("**/api/performance/trace", async (route) => {
        batches.push(route.request().postDataJSON());
